...
const onValidationInputs = () => {
  ...
  inputsValues.forEach((input) => {
    ...
    input.dataset.js === "mainImg" && validateFile(input, valuesInput);

    input.dataset.js === "altImgMain" &&
      validateMinCaracter(input, valuesInput, 10);
...
const groupBodygroup = document.querySelector("[data-group='bodygroup']");
...
onValidationDinamicInputs(groupBodygroup, bodygroup);
...
const handleSubmite = (obj) => {
  ...
  btnSubmite.addEventListener("click", () => {
    ...
    getAllReferenceErrors(bodygroup);
